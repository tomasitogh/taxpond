...
| ---- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| 1.1  | Instalar `@duckdb/duckdb-wasm`, `@duckdb/duckdb-wasm-binding` y `fflate`                                                    | `package.json`                                                |
| 1.2  | Configurar `next.config.ts` con `experiments.asyncWebAssembly = true` en webpack                                            | `next.config.ts`                                              |
| 1.3  | Crear singleton `getDuckDB()` con lazy loading via `dynamic import()` (~30 MB WASM bundle) + React Context `DuckDBProvider` | `src/lib/duckdb/connection.ts`, `src/lib/duckdb/provider.tsx` |
...
- CSP headers pueden romper DuckDB-WASM en producción (necesita Worker loading). Verificar la config de despliegue.
...
| 2.4  | Manejar encoding: probar UTF-8 y Latin-1. Usar opción `ENCODING='latin1'` de DuckDB si es necesario                                                  | `src/lib/duckdb/csv-loader.ts`                   |
