    ...
    <div className="flex min-w-[150px] flex-col gap-1">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className="border-border text-foreground hover:bg-accent inline-flex h-7 w-full items-center justify-between gap-1 rounded-md border bg-transparent px-2 text-xs font-medium">
          {COLUMN_TYPES.find((t) => t.value === currentType)?.label}
          <ChevronDown className="h-3 w-3 shrink-0" />
        </DropdownMenuTrigger>
        <DropdownMenuContent align="start">
          {COLUMN_TYPES.map((t) => (
            <DropdownMenuItem key={t.value} onClick={() => onTypeChange(column, t.value)}>
              <span className="flex w-full items-center justify-between gap-2">
                {t.label}
                {currentType === t.value && <Check className="h-3 w-3" />}
              </span>
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      ...
      <div className="flex gap-1">
        ...
        <DropdownMenu onOpenChange={(open) => loadDistinctValues(open)}>
          <DropdownMenuTrigger className="border-border text-foreground hover:bg-accent inline-flex h-7 max-w-[110px] items-center gap-1 rounded-md border bg-transparent px-2 text-xs font-medium">
            <span className="truncate">{currentFilter ?? 'Filter by'}</span>
            <ChevronDown className="h-3 w-3 shrink-0" />
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="max-h-60 overflow-auto">
            <DropdownMenuItem onClick={() => onFilterSelect(column, null)}>All</DropdownMenuItem>
